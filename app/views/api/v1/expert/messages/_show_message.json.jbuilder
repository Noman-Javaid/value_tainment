json.extract! message,
              :id,
               ...
               :answer_type,
               :attachment_id
json.status message.status == 'sent' && message.sender_id != @expert.id ? 'pending' : message.status
json.sent message.sent_at(for_user: current_user)
json.expiration_status message.expiration_status(for_user: @expert)
json.amount message.price
json.has_read message.read_by_user?(@expert)
