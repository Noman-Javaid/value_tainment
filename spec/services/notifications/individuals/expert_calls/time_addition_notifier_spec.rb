...
describe Notifications::Individuals::ExpertCalls::TimeAdditionNotifier do
  ...
  let(:message) { 'Confirmation/Rejection Addition Time' }
  ...
  context 'when individual allow notifications' do
    before do
      time_addition.expert_call.individual.user.update!(allow_notifications: true)
    end

    context 'when individual has an Android phone' do
      let(:device) do
        create(:device, :with_android, user: time_addition.expert_call.individual.user)
      end
    ...
    context 'when individual has an iOS phone' do
      let(:device) do
        create(:device, :with_ios, user: time_addition.expert_call.individual.user)
      end
  ...
  context 'when individual do not allow notifications' do
    before do
      time_addition.expert_call.individual.user.update!(allow_notifications: false)
    end
